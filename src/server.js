...
export const getDb = (readOnly = true) => {
    ...
    const db = new sqlite.Database(dbPath, mode, (err) => {
        if (err) {
            // surface error early
...
server.registerResource('table-schema', new ResourceTemplate('schema://table/{tableName}', {
    list: async () => {
        const db = getDb();
        try {
    ...
    const db = getDb();
    try {
        const result = await db.all("SELECT sql FROM sqlite_master WHERE type='table' AND name = ?", [tableName]);
        if (result.length === 0) {
...
server.registerPrompt('query-table', {
    title: 'Query Table',
    ...
    argsSchema: {
        ...
        columns: z
            .string()
            .optional()
            .describe('Comma-separated list of columns to select. If not provided, all columns will be selected.'),
        filter: z
            .string()
            .optional()
            ...
            messages: [
                {
                    ...
                    content: {
                        type: 'text',
                        text: `Error querying table ${tableName}: ${err.message}`,
                    },
                },
            ],
        };
...
server.registerTool('listTables', {
    title: 'List Tables',
    description: 'Lists all tables in the database, returning links to their schemas.',
    inputSchema: {
        cursor: z
            .string()
            .optional()
    ...
    try {
        ...
        const nextCursor = end < tables.length ? (page + 1).toString() : undefined;
        return {
            content: [
    ...
    try {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: `Error creating table '${tableName}': ${err.message}`,
                },
            ],
            isError: true,
        };
...
const executeModificationTool = server.registerTool('executeModification', {
    ...
    try {
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: `Error executing ${operation} on table ${tableName}: ${err.message}`,
                },
            ],
            isError: true,
        };
...
server.registerTool('adminLogin', {
    title: 'Admin Login',
